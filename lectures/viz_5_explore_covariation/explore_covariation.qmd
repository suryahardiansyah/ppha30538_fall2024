...
alt.Chart(penguins).mark_bar().encode(
    alt.X('year:O', bin=alt.BinParams(maxbins=20)),
    alt.Y('body_mass_g:Q', bin=alt.BinParams(maxbins=20)),
    alt.Color('count()')
) 
```
...
alt.Chart(penguins).mark_rect().encode(
    alt.X('year:O', bin=alt.Bin(maxbins=70)), 
    alt.Y('body_mass_g:Q', bin=alt.Bin(maxbins=70)),
    alt.Color('count()', scale=alt.Scale(scheme='blues')))
```
